...
def validate_inputs(test_data):
    ...
    required_fields = ["Endpoint", "DateRetrieved", "FullName", "NPI", "FirstName",
        "LastName", "Gender", "Taxonomy", "GroupName", "ADD1",
        "City", "State", "Zip", "Phone", "Fax", "Email",
        "lat", "lng", "LastPracUpdate", "LastPracRoleUpdate",
        "LastLocationUpdate", "AccuracyScore"]
    ...
    for field in required_fields:
        ...
        value = test_data.get(field, "").strip()  # Get the value of the field or None if not present
        if not value:
            missing_fields.append(field)

    if missing_fields:
            return {"success": False, "message": f"Missing required fields: {', '.join(missing_fields)}", "status_code": 400}
    ...
    for field, value in test_data.items():
        ...
        if field == "DateRetrieved":
            try:
                ...
                return {"success": False, "message": f"Invalid date format for `{field}`. Entry must be MM-DD-YYYY.", "status_code": 400}

        # NPI length validation
        if field == "NPI" and len(value) != npi_length:
            return {"success": False, "message": f"NPI must be exactly {npi_length} digits.", "status_code": 400}

        # Phone number format validation
        if field == "Phone" and not re.match(phone_pattern, value):
            return {"success": False, "message": f"Invalid phone number format for `{field}`.", "status_code": 400}

        # Email format validation
        if field == "Email" and not re.match(email_pattern, value):
            return {"success": False, "message": f"Invalid email format for `{field}`." , "status_code": 400}
    ...
    if "ADD2" in test_data and not test_data["ADD1"]:
        return {"success": False, "message": "Address line 2 provided without Address line 1.", "status_code": 400}
